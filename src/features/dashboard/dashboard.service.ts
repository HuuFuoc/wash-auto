...
import { VoucherStatusEnum } from '../voucher/types/voucher-status.enum';
...
import {
  ...
  NamedRevenue,
  RankRow,
} from './types/dashboard-report.type';
 ...
 *
 * Every figure is aggregated live from the real collections - orders,
 * vehicles, vouchers, work_orders, users, loyalty_accounts, staff_shifts.
 * Revenue is recognised ONLY on orders that are both `completed` AND `paid`;
 * cancelled / no-show / unpaid orders never contribute. Refunds are tracked
 * via `payment_status = refunded` and subtracted from net revenue.
 */
@Injectable()
export class DashboardService {
  ...
  async getReport(query: QueryDashboardDto): Promise<DashboardReport> {
    ...
    // expects, regardless of how the raw instant is stored (UTC) in Mongo.
  ...
  // ─── Vehicles (not date-bound - a vehicle exists regardless of window) ──
  ...
  private assemble(input: AssembleInput): DashboardReport {
    ...
    const netGross = rev?.net ?? 0; // sum of amounts actually charged
...
/** Start of the VN calendar day for a `YYYY-MM-DD` (or ISO) date string. */
function vnDayStart(dateStr: string): Date {
  return new Date(`${dateStr.slice(0, 10)}T00:00:00.000${VN_OFFSET}`);
}

/** End of the VN calendar day for a `YYYY-MM-DD` (or ISO) date string. */
