...
export class EmailService implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    this.transporter = nodemailer.createTransport({
      ...
      secure: this.config.getOrThrow<boolean>('email.secure'),
      auth: {
        user: this.config.getOrThrow<string>('email.user'),
        pass: this.config.getOrThrow<string>('email.pass'),
      },
      pool: true,
      maxConnections: 3,
      maxMessages: 100,
    ...
    this.transporter
      .verify()
      .then(() => this.logger.log('SMTP transporter ready'))
      .catch((err: Error) =>
  ...
  async sendOtpEmail(
    ...
    try {
      ...
      this.logger.error(`SMTP send failed (otp) to=${to} reason=${msg}`);
      throw err;
    }
  }
  ...
  async sendOrderConfirmationEmail(
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      this.logger.error(
        `SMTP send failed (order) to=${to} orderId=${ctx.orderId} reason=${msg}`,
      );
      throw err;
    }
  }
