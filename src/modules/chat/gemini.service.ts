...
// Business logic copied verbatim from features/chat/services/gemini.service.ts;
// onModuleInit → constructor, ConfigService → config, Logger → console.
export class GeminiService {
  ...
  constructor(private readonly tools: ChatToolsService) {
    ...
    console.log(`Gemini client ready (model=${this.model})`);
  }
  ...
  /** Single Gemini call with one short retry on 429, then a friendly 503. */
    ...
    try {
      ...
      if (status === 429) {
        ...
        if (delayMs > 0 && delayMs <= 5_000) {
          console.warn(`Gemini 429 - retrying once after ${delayMs}ms`);
  ...
  private wrapQuotaError(err: unknown): ServiceUnavailableException {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`Gemini quota exhausted: ${msg}`);
