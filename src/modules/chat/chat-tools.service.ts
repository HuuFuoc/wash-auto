...
/** Context the bot has about the caller. `customerId` is undefined for guests. */
...
// features/chat/services/chat-tools.service.ts; only DI + Logger were swapped.
export class ChatToolsService {
  ...
  /** Tool catalogue advertised to Gemini (descriptions in Vietnamese). */
  ...
  async execute(
    ...
    try {
      switch (name) {
        ...
        default:
          return { data: null, error: `Tool "${name}" không tồn tại.` };
      }
    } catch (err) {
